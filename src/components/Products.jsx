...
const si = { hidden: { opacity: 0, y: 50, scale: 0.9 }, visible: { opacity: 1, y: 0, scale: 1, transition: { duration: 0.6, ease: [0.22, 1, 0.36, 1] } } }
    ...
    <TiltCard>
      <motion.div variants={si} className="group cursor-pointer" onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} onClick={() => onProductSelect(p)}>
        <div className="aspect-square bg-gray-100 rounded-2xl overflow-hidden mb-3 relative">
          ...
          <motion.div animate={{ x: ['-100%', '200%'] }} transition={{ duration: 1.5, repeat: Infinity, repeatDelay: 3 }}
          ...
          <motion.div className="w-full h-full flex items-center justify-center" animate={{ scale: hovered ? 1.05 : 1 }} transition={{ duration: 0.4 }}>
            <motion.div className="w-24 h-24 rounded-2xl shadow-lg" style={{ backgroundColor: p.colors[0].hex }}
              animate={{ rotate: hovered ? [0, 5, -5, 0] : 0, scale: hovered ? 1.15 : 1 }} transition={{ duration: 0.5 }} />
          </motion.div>
          ...
          <div className="absolute top-2.5 left-2.5 flex flex-col gap-1">
            {discount > 0 && (
              <motion.span initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }}
                className="px-2 py-0.5 text-[10px] font-bold rounded-full bg-red-500 text-white shadow-md">-{discount}%</motion.span>
            )}
            {p.tags.includes('new') && (
              <motion.span initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.1 }}
                className="px-2 py-0.5 text-[10px] font-bold rounded-full bg-blue-500 text-white shadow-md">New</motion.span>
            )}
            {p.tags.includes('bestseller') && !p.tags.includes('new') && (
              <motion.span initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.1 }}
                className="px-2 py-0.5 text-[10px] font-bold rounded-full bg-gray-900 text-white shadow-md">Best</motion.span>
            )}
          </div>

          {/* Quick Add */}
          <motion.div initial={false} animate={{ opacity: hovered ? 1 : 0, y: hovered ? 0 : 10 }} transition={{ duration: 0.25 }}
          ...
          {/* Hover glow */}
          <motion.div initial={false} animate={{ opacity: hovered ? 0.6 : 0 }}
            className="absolute inset-0 bg-gradient-to-t from-gray-900/40 via-gray-900/10 to-transparent pointer-events-none" />
        </div>

        <div className="space-y-1.5">
          <div className="flex items-center gap-1.5">
            <motion.div animate={{ scale: hovered ? 1.1 : 1 }} className="flex text-amber-400 text-[10px]">{'★'.repeat(Math.floor(p.rating))}</motion.div>
          ...
          <div className="flex items-center gap-2">
            <motion.span animate={{ scale: hovered ? 1.05 : 1 }} className="text-sm font-bold text-gray-900">{convertPrice(p.price, currency)}</motion.span>
            {p.originalPrice && <span className="text-xs text-gray-400 line-through">{convertPrice(p.originalPrice, currency)}</span>}
          </div>
          <div className="flex gap-1.5 pt-0.5">
            {p.colors.slice(0, 4).map((c) => (
              <motion.span key={c.name} whileHover={{ scale: 1.4, y: -2 }}
                className="w-3.5 h-3.5 rounded-full border border-gray-200 transition-transform cursor-pointer shadow-sm" style={{ backgroundColor: c.hex }} title={c.name} />
            ))}
          </div>
        </div>
      </motion.div>
...
function ProductListItem({ p, currency, onProductSelect, onAddToCart }) {
  const discount = p.originalPrice ? Math.round((1 - p.price / p.originalPrice) * 100) : 0
  return (
    <motion.div variants={si} whileHover={{ x: 6, scale: 1.005, boxShadow: '0 10px 40px -10px rgba(0,0,0,0.1)' }}
      className="flex gap-4 p-3 bg-gray-50 rounded-xl cursor-pointer hover:bg-white hover:shadow-xl transition-all duration-300 border border-transparent hover:border-gray-100"
      onClick={() => onProductSelect(p)}>
      <motion.div whileHover={{ scale: 1.08 }} className="w-28 h-28 bg-gray-200 rounded-xl flex items-center justify-center flex-shrink-0 overflow-hidden">
        <div className="w-14 h-14 rounded-xl shadow-md" style={{ backgroundColor: p.colors[0].hex }} />
      </motion.div>
      <div className="flex-1 min-w-0 grid grid-cols-[1fr_auto] gap-2">
        ...
        <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
          onClick={(e) => { e.stopPropagation(); onAddToCart(p) }}
          className="self-center px-3 py-1.5 bg-gray-900 text-white text-xs font-medium rounded-lg hover:bg-gray-800 transition-all flex-shrink-0 whitespace-nowrap shadow-md">
          Add to Cart
        </motion.button>
      </div>
    </motion.div>
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <motion.h2 initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="text-xl font-bold text-gray-900">All Products</motion.h2>
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }}
            className="text-xs text-gray-500 mt-0.5">{items.length} products found</motion.p>
        </div>
        <div className="flex items-center gap-2">
          <motion.select whileTap={{ scale: 0.95 }} value={filters.sortBy}
            onChange={(e) => window.dispatchEvent(new CustomEvent('filterChange', { detail: { sortBy: e.target.value } }))}
            className="text-xs border border-gray-200 rounded-lg px-3 py-2 bg-white focus:outline-none focus:ring-2 focus:ring-gray-900 cursor-pointer transition-all hover:border-gray-300">
            {SORT.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
          </motion.select>
          <div className="flex items-center bg-gray-100 rounded-lg p-0.5">
            {['grid', 'list'].map((m) => (
              <motion.button key={m} whileTap={{ scale: 0.9 }} onClick={() => setView(m)}
                className={`p-1.5 rounded-md transition-all ${view === m ? 'bg-white shadow-sm text-gray-900' : 'text-gray-400 hover:text-gray-600'}`}>
                {m === 'grid'
      ...
      <AnimatePresence mode="wait">
        {items.length === 0 ? (
          <motion.div key="empty" initial={{ opacity: 0, scale: 0.8, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.8 }}
            className="text-center py-20">
            <motion.div animate={{ y: [0, -8, 0], rotate: [0, 5, -5, 0] }} transition={{ duration: 3, repeat: Infinity }}
              className="w-20 h-20 mx-auto bg-gray-100 rounded-full flex items-center justify-center mb-4 shadow-inner">
